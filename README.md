...
The step check allows simple testing of conditions. For professional test automation, CasperJS comes with a tester module for unit and functional testing purpose. To use advanced testing features, run TA.Gui with the test option.
...
Besides integrating with other web applications, the API can be extended to integrate with hardware (for eg Arduino or Raspberry Pi) for physical world interactions and machine learning service providers for deep fuzzy thinking capacity. 
