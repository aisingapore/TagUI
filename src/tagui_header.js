...
function teleport_distance(teleport_marker) {
    ...
    if (teleport_marker.indexOf('[BREAK_SIGNAL]') > -1) {
        for (s = casper.steps.length - 1; s >= 0; s--) {
            if (casper.steps[s].toString() == ("function () {for_loop_signal = '" + teleport_marker + "';}")) { number_to_hop = s; break; }
        };
        ...
        if (number_to_hop == 0) {
            for (s = casper.steps.length - 1; s >= 0; s--) {
                if (casper.steps[s].toString() ==
                    ("function () {for_loop_signal = '" + teleport_marker.replace('[CONTINUE_SIGNAL]', '[BREAK_SIGNAL]') + "';}")) { number_to_hop = s; break; }
            };
        }
...
function techo(echo_string) {
    if (!quiet_mode) { // mute about:blank, eg for desktop automation
        if ((echo_string == 'about:blank - \n') || (echo_string == '\nabout:blank - ')) casper.echo('');
        else if (tagui_language.toLowerCase() == 'english') casper.echo(echo_string);
        else {
            var translated_string = translate(echo_string, 'to', tagui_language.toLowerCase()); casper.echo(translated_string);
            if (translated_string.indexOf('ERROR - translation engine') !== -1) casper.exit();
        }
    } return;
}
...
function save_table(file_name, selector) {
    ...
    for (table_row = 1; table_row <= 1024; table_row++) {
        row_data = ""; for (table_col = 1; table_col <= 1024; table_col++) {
...
function read_excel(input_excel) { // for reading from excel target
    if (user_system == 'windows') {
        ...
        casper.waitForExec('cscript excel_steps.vbs //NoLogo', null, function (response) {
            excel_result = '';
            ...
            var range_size = excel_range_to_size(cell_range); if (range_size[0] > 1 || range_size[1] > 1) {
                ...
                excel_result = excel_array;
            } else {
                excel_result = excel_result.split('[EXCEL_DELIMITER]')[0];
                if (excel_result && !isNaN(excel_result)) excel_result = Number(excel_result);
            }
            excel_json = response.data;
        }, function () { this.echo('ERROR - Excel automation exceeded ' + (3 * casper.options.waitTimeout / 1000).toFixed(1) + 's timeout').exit(); }, (3 * casper.options.waitTimeout));
    }
        ...
        casper.waitForExec('osascript excel_steps.scpt', null, function (response) {
            excel_result = '';
            excel_result = (response.data.stdout.trim() || response.data.stderr.trim());
            if (excel_result.indexOf('ERROR - cannot find Excel sheet') !== -1) casper.echo(excel_result).exit();
            var range_size = excel_range_to_size(cell_range); if (range_size[0] > 1 || range_size[1] > 1) {
...
function size_to_excel_range(cell, width, height) { // for converting array to range
    ...
    for (count = 0; count < width - 1; count++) {
        ...
        if (mid_char == '-') { mid_char = 'A'; continue; }
            ...
            mid_char = String.fromCharCode(mid_char.charCodeAt() + 1); if (mid_char != '[') continue; else mid_char = 'A';
...
function write_excel(output_excel) { // for writing to excel target
    if (user_system == 'windows') {
        ...
        if (!Array.isArray(excel_result)) {
            ...
            var range_size = [excel_result[0].length, excel_result.length];
            ...
            for (row = 0; row < range_size[1]; row++) {
                for (col = 0; col < range_size[0]; col++) {
...
function excel_retrieve(right_param) { // check OS before starting Excel automation
    ...
    if (right_param.match(/\[.*\.(xl.|xl..|xml|csv)\].*![A-Z0-9]*/i) == null) { excel_result = ''; excel_result = eval(right_param); }
    else if (excel_result == '[LIVE_MODE]') {
        excel_result = '';
...
// broken into 2 functions for excel_result to be usable with CasperJS structure
function excel_assign(left_param) {
    if (left_param.match(/\[.*\.(xl.|xl..|xml|csv)\].*![A-Z0-9]*/i) == null)
...
function word_retrieve(right_param) { // check OS before starting Word automation
    ...
    if (right_param.match(/\[.*\.(doc|docx|docm|dot|dotx)\]/i) == null) { word_result = ''; word_result = eval(right_param); }
    else if (word_result == '[LIVE_MODE]') {
        word_result = '';
...
// broken into 2 functions for word_result to be usable with CasperJS structure
function word_assign(left_param) {
    if (left_param.match(/\[.*\.(doc|docx|docm|dot|dotx)\]/i) == null)
...
function pdf_retrieve(right_param) { // check OS before starting PDF automation
    ...
    if (right_param.match(/\[.*\.pdf\]/i) == null) { pdf_result = ''; pdf_result = eval(right_param); }
    else if (pdf_result == '[LIVE_MODE]') {
        pdf_result = '';
...
// broken into 2 functions for pdf_result to be usable with CasperJS structure
function pdf_assign(left_param) {
    if (left_param.match(/\[.*\.pdf\]/i) == null)
...
function translate(script_line, direction, language) {
    var start_keywords = '|click|rclick|dclick|tap|move|hover|' +
...
function tx(locator) {
    if (is_xpath_selector(locator)) return xps666(locator.replace(/'/g, '"'));
...
function check_tx(locator) {
    if (is_xpath_selector(locator)) { if (casper.exists(xps666(locator.replace(/'/g, '"')))) return true; else return false; }
...
function present(element_locator) {
    ...
    if (is_sikuli(element_locator)) {
        var abs_param = abs_file(element_locator); var fs = require('fs');
...
function appeared(element_identifier) {
    var appeared_timeout = Date.now() + casper.options.waitTimeout;
...
clipboard = function (clipboard_text) { // use this function declaration style for sikuli detection in tagui_parse.php
    if (!clipboard_text) {
        ...
        vision_step('clipboard_text = ucode("' + escape_bs(clipboard_text) + '")'); // escape_bs() for \n \t etc
...
function get_text(source_text, left_marker, right_marker, optional_count) {
    ...
    if (optional_count > 1) {
        var occurrence_count = 2; while (occurrence_count <= optional_count) {
            occurrence_count++;
...
if (chrome_id > 0) { // super large if block to load chrome related functions if chrome or headless option is used
    ...
    chrome.elementVisible = function (selector) { // same as chrome.exists, except for checking visibility
        ...
        try {
            var ws_json = JSON.parse(ws_message); if (ws_json.result.result.value == true)
    ...
    chrome.countElements = function (selector) { // same as chrome.exists, except element count is returned
        ...
        try {
            var ws_json = JSON.parse(ws_message); if (ws_json.result.result.value > 0)
    ...
    chrome.mouse.getXY = function (selector) { // helper function to get xy center coordinates of selector
        ...
        try {
            var ws_json = JSON.parse(ws_message); if (ws_json.result.result.value.x > 0 && ws_json.result.result.value.y > 0) {
    ...
    chrome.getRect = function (selector) { // helper function to get rectangle boundary coordinates of selector
        ...
        else var ws_message = chrome_step('Runtime.evaluate', { expression: 'var result_bounds = ' + chrome_context + '.querySelector(\'' + selector + '\').getBoundingClientRect(); var result_rect = {top: Math.round(result_bounds.top), left: Math.round(result_bounds.left), width: Math.round(result_bounds.width), height: Math.round(result_bounds.height)}; result_rect', returnByValue: true }); try {
    ...
    chrome.fetchText = function (selector) { // grab text from selector following casperjs logic, but grab only first match
        ...
        try {
            var ws_json = JSON.parse(ws_message); if (ws_json.result.result.value)
    ...
    chrome.decode = function (str) { // funtion to convert base64 data to binary string
        ...
        var c1, c2, c3, c4, i = 0, len = str.length, out = ""; while (i < len) {
            ...
            while (i < len && c4 === -1); if (c4 === -1) { break; } out += String.fromCharCode((c3 & 0x03) << 6 | c4);
        } return out;
    };
    ...
    chrome.evaluate = function (fn_statement, eval_json) { // evaluate expression in browser dom context
        ...
        try {
            var ws_json = JSON.parse(ws_message); if (ws_json.result.result.value)
                return ws_json.result.result.value; else return null;
        } catch (e) { return null; }
    };
    ...
    chrome.withFrame = function (frameInfo, then) { // replace casperjs frame for switching frame context
        ...
        casper.then(function _step() { // track frame (x,y) offset so that steps within frame work correctly
            ...
            chrome_step('Runtime.evaluate', { expression: new_context + ' = document.evaluate(\'(//frame|//iframe)[@name="' + frameInfo + '" or @id="' + frameInfo + '"]\',' + chrome_context + ',null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null).snapshotItem(0).contentDocument' }); chrome_context = new_context;
        }); // set mainframe_context/subframe_context in dom
        casper.then(then); casper.then(function _step() {
            if (chrome_context == 'subframe_context') {
                ...
                chrome_step('Runtime.evaluate', { expression: 'mainframe_context = null' });
                chrome_context = 'document'; frame_step_offset_x = 0; frame_step_offset_y = 0;
            }
        });
    ...
    chrome.waitForPopup = function (popupInfo, then, onTimeout) { // replace casperjs waitforpopup for checking popup window
        casper.waitFor(function check() { // use similar logic as chrome withpopup to scan through list of browser targets
            ...
            try {
                var ws_json = JSON.parse(ws_message); if (ws_json.result.targetInfos) chrome_targets = ws_json.result.targetInfos;
                else chrome_targets = [];
            } catch (e) { chrome_targets = []; } // following line scan through targets to find match
    ...
    chrome.withPopup = function (popupInfo, then) { // replace casperjs withpopup for handling popup window
        casper.then(function _step() { // get list of targets, find a match, attach to the target and set chrome_targetid
            ...
            if (found_targetid !== '') {
                var ws_message = chrome_step('Target.attachToTarget', { targetId: found_targetid });
                try {
                    var ws_json = JSON.parse(ws_message); if (ws_json.result.sessionId !== '')
                        found_targetid = ws_json.result.sessionId; else found_targetid = '';
                } catch (e) { found_targetid = '' };
            }
            chrome_targetid = found_targetid;
        }); // set chrome_targetid only after attaching to found target successfully
        casper.then(then); casper.then(function _step() {
            if (chrome_targetid !== '') // detach from target after running then
    ...
    chrome.getCurrentUrl = function () { // get url of current webpage
        ...
        try {
            ...
            if (ws_json.result.result.value) return ws_json.result.result.value; else return '';
        } catch (e) { return ''; }
    };
...
function tagui_parse(raw_input) {
    if (tagui_language.toLowerCase() == 'english') return parse_intent(raw_input);
        ...
        var translated_string = translate(raw_input, 'from', tagui_language.toLowerCase());
...
function escape_quote(script_line) { // helper function for string context intents
    ...
    for (char_counter = 0; char_counter < script_line.length; char_counter++) {
        if ((script_line.charAt(char_counter) == "'") && (current_context == 'string')) { script_line = script_line.substr(0, char_counter) + '\\\'' + script_line.substr(char_counter + 1); char_counter++; }
        if (script_line.charAt(char_counter) == "`") { if (current_context == 'string') current_context = 'code'; else current_context = 'string'; }
    } return script_line;
}
    ...
    for (char_counter = 0; char_counter < script_line.length; char_counter++) {
        ...
        if (quote_token == ")+[SINGLE_QUOTE_FOR_VARIABLE_HANDLING]") { // protect code context from escape_bs()
            if (script_line.charAt(char_counter) == "'") script_line = script_line.substr(0, char_counter) +
                ...
                "[DOUBLE_QUOTE_FOR_VARIABLE_HANDLING]" + script_line.substr(char_counter + 1);
        }
    } return script_line;
}
...
function is_excel(raw_intent) {
    if (raw_intent.indexOf('=') == -1) return false;
...
function url_intent(raw_intent) {
    if (raw_intent.toLowerCase().substr(0, 4) == 'www.') raw_intent = 'https://' + raw_intent;
...
function type_intent(raw_intent) {
    ...
    if ((param1 == '') || (param2 == '')) return "this.echo('ERROR - target/text missing for " + raw_intent + "')";
        ...
        if (param2.indexOf('[clear]') == 0) {
            if (param2.length > 7) param2 = param2.substr(7); else param2 = "";
...
function save_intent(raw_intent) {
    ...
    if (params == '') return "this.echo('ERROR - target missing for " + raw_intent + "')";
        ...
        if (check_tx(params)) return "save_text('',this.fetchText(tx('" + params + "')).trim())";
        else return "this.echo('ERROR - cannot find " + params + "')";
    }
}
...
function snap_intent(raw_intent) {
    ...
    if (params == '') return "this.echo('ERROR - target missing for " + raw_intent + "')";
        ...
        if (check_tx(params)) return "this.captureSelector(snap_image(),tx('" + params + "'))";
        else return "this.echo('ERROR - cannot find " + params + "')";
    }
}

function table_identifier(given_identifier) {
    if (!given_identifier) return '';
...
function table_intent(raw_intent) {
    ...
    if (params == '') return "this.echo('ERROR - target missing for " + raw_intent + "')";
        ...
        if (check_tx(table_identifier(param1)))
        ...
        if (check_tx(table_identifier(params))) return "save_table('',tx('" + table_identifier(params) + "'))";
...
function excel_intent(raw_intent) {
    raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
    if (user_system != 'windows' && user_system != 'mac') { return "excel_result = ''; this.echo('ERROR - Excel automation only for Windows and Mac')"; }
...
function word_intent(raw_intent) {
    raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
    if (user_system != 'windows' && user_system != 'mac') { return "word_result = ''; this.echo('ERROR - Word automation only for Windows and Mac')"; }
...
function pdf_intent(raw_intent) {
    raw_intent = eval("'" + escape_bs(raw_intent) + "'"); // support dynamic variables
    if (user_system != 'windows' && user_system != 'mac') { return "pdf_result = ''; this.echo('ERROR - PDF automation only for Windows and Mac')"; }
...
casper.selectOptionByValue = function (selector, valueToMatch) { // solution posted in casperjs issue #1390
    this.evaluate(function (selector, valueToMatch) {
        ...
        evt.initUIEvent("change", true, true); select.dispatchEvent(evt);
...
phantom.onError = function (msg, trace) {
    if (msg.indexOf('Error:') !== -1) { // clean up and format error message
